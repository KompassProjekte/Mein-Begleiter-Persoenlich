...
(() => {
  ...
  // Qualitätsversion 1.9.1.2.5.9: eindeutige Eingabewege statt versteckter Umwege.
  ...
  if (einstellungen && !q("#v19129Ueber", einstellungen)) {
    ...
    ueber.innerHTML = `<h3>Über Mein Begleiter</h3><p><strong>Version ${VERSION} PWA – PERSÖNLICHE ARBEITSVERSION</strong><br>Qualitätsversion · Stand: 01.09.2026<br>Entwickelt von Lothar &amp; Nimbus</p><p>Die Gesundheitsdaten werden lokal auf diesem Gerät gespeichert. Es findet keine automatische Synchronisation statt.</p>`;
  ...
  kostenDialog.id = "v19129KostenDialog";
  kostenDialog.className = "dialog v19129-kostendialog";
  kostenDialog.innerHTML = `<form method="dialog"><div class="dialog-kopf"><div><h2>Kosten eintragen</h2><p>Rechnung, Rezept oder Fahrtkosten als eigenen Kostenposten speichern.</p></div><button type="button" class="dialog-x" data-kosten-abbruch aria-label="Dialog schließen">×</button></div><div class="dialog-inhalt v19129-kostenformular"><label>Datum<input id="v19129KostenDatum" type="date" required></label><label>Uhrzeit<input id="v19129KostenZeit" type="time" required></label><label>Kostenart<select id="v19129KostenArt" required><option value="rechnung">Rechnung</option><option value="rezept">Rezept</option><option value="fahrt">Fahrtkosten</option></select></label><label>Betrag in Euro<input id="v19129KostenBetrag" type="number" min="0.01" step="0.01" inputmode="decimal" required></label><label class="span2">Bezeichnung / Empfänger<input id="v19129KostenText" maxlength="160" placeholder="z. B. Praxis, Apotheke oder Fahrt zur Behandlung" required></label><label class="span2">Arzt / Stelle (freiwillig)<input id="v19129KostenArzt" maxlength="100" list="arztVorschlaege"></label><label class="span2">Notiz (freiwillig)<textarea id="v19129KostenNotiz" rows="2" maxlength="240"></textarea></label></div><div class="dialog-fuss"><button type="button" class="knopf sekundaer" data-kosten-abbruch>Abbrechen</button><button type="button" class="knopf" id="v19129KostenSpeichern">Kosten speichern</button></div></form>`;
  document.body.appendChild(kostenDialog);
  const kostenOeffnen = () => {
    q("#v19129KostenDatum").value = heute();
    q("#v19129KostenZeit").value = new Date().toTimeString().slice(0,5);
    q("#v19129KostenBetrag").value = "";
    q("#v19129KostenText").value = "";
    q("#v19129KostenArzt").value = "";
    q("#v19129KostenNotiz").value = "";
  ...
  q("#v19129KostenSpeichern").addEventListener("click",()=>{
    ...
    if(!datum||!zeit||!art||!Number.isFinite(betrag)||betrag<=0||!bezeichnung){toast("Bitte Datum, Uhrzeit, Kostenart, Betrag und Bezeichnung eintragen.");return;}
    const id=`j${Date.now()}-${Math.random().toString(36).slice(2,7)}`;
    const e=typeof leeresEintragObjekt==="function"?leeresEintragObjekt(id,datum,zeit):{id,datum,messUhrzeit:zeit,arzt:"",massnahme:"",bemerkung:"",rechnung:0,rezept:0,fahrt:0};
    e.eintragsart="kosten"; e.arzt=q("#v19129KostenArzt").value.trim(); e.massnahme=bezeichnung; e.bemerkung=q("#v19129KostenNotiz").value.trim(); e[art]=betrag;
    daten.eintraege.push(e); speichern(); renderAlles(); kostenDialog.close(); toast("Kostenposten gespeichert");
  });
