...
(() => {
  ...
  const vitalButton = q("#v19127VitalButton") || document.createElement("button");
  vitalButton.type = "button"; vitalButton.id = "v19127VitalButton"; vitalButton.className = "knopf v19125-vitalbutton"; vitalButton.innerHTML = "＋ Vitalwerte eintragen";
  ...
  seite.innerHTML = `<button class="knopf sekundaer zur-uebersicht" type="button" data-zur>← Zur Übersicht</button><div class="v19121-seitenkopf"><div><h2>Druckzentrum</h2><p>Wählen Sie den Inhalt aus. Es werden nur die dazu passenden Einstellungen angezeigt.</p></div></div><article class="karte"><h3>Was möchten Sie drucken?</h3><div id="v19125Druckarten"></div><div class="v19125-druckfilter"><label>Von<input class="filter" id="v19125Von" type="date"></label><label>Bis<input class="filter" id="v19125Bis" type="date"></label><label id="v19125ArztLabel">Arzt / Stelle<select class="filter" id="v19125Arzt"><option value="">Alle Ärzte / Stellen</option></select></label><label>Druckdichte<select class="filter" id="v19125Dichte"><option value="lesbar">Gut lesbar – empfohlen</option><option value="sparsam">Papiersparend</option></select></label></div><fieldset id="v19125DiagrammBox" class="v19125-zusatz" hidden><legend>Diagramme auswählen</legend><div class="v19123-diagrammauswahl" id="v19125Diagramme"></div></fieldset><div class="v19125-format" id="v19125Format"></div><div class="v19123-druckaktionen"><button class="knopf" type="button" id="v19125Drucken">🖨 Drucken oder als PDF speichern</button></div><p class="hinweis">Im Druckfenster können Sie einen Drucker auswählen oder die Ausgabe als PDF speichern.</p></article><article class="v19123-vorschau" id="v19125Vorschau" aria-live="polite"></article>`;
  q("main").appendChild(seite);
  q("[data-zur]", seite).addEventListener("click", () => typeof wechsleSeite === "function" && wechsleSeite("cockpit"));
  const arten = [["fragen","Offene Fragen"],["kommend","Kommende Termine mit Fragen"],["vergangen","Vergangene Termine"],["alletermine","Alle Termine"],["medikamente","Medikamentenplan"],["behandlung","Behandlung & Medikamente"],["nebenwirkungen","Beobachtete Nebenwirkungen"],["schritte","Behandlungsschritte"],["dokumente","Liste meiner Dokumente"],["verlauf","Verlauf"],["diagramme","Ausgewählte Diagramme"],["tagebuch","Tagebuch"],["buch","Persönliches Buch"]];
  const druckGruppen=[["Termine und Fragen",["kommend","fragen","vergangen","alletermine"]],["Behandlung",["medikamente","behandlung","nebenwirkungen","schritte"]],["Dokumentation und Verlauf",["dokumente","verlauf","diagramme"]],["Persönliche Ausgaben",["tagebuch","buch"]]];
  ...
  const vorschau = async () => {
    ...
    q("#v19125ArztLabel").hidden=["verlauf","diagramme","medikamente","tagebuch","buch"].includes(a);
    ...
    out.className=`v19123-vorschau${q("#v19125Dichte").value==="sparsam"?" v19123-sparsam":""}${quer?" v19125-quer":" v19125-hoch"}`; out.innerHTML=(a==="buch"?"":kopf(titel))+html;
  };
  ...
  q("#v19125Drucken").addEventListener("click", async()=>{await vorschau();const quer=["verlauf","diagramme"].includes(art());ausrichtungsStil.textContent=`@media print{@page{size:A4 ${quer?"landscape":"portrait"};margin:${quer?"12mm":"14mm"}}`;document.body.classList.toggle("v19125-print-quer",quer);document.body.classList.add("v19125-druckt");await new Promise(r=>requestAnimationFrame(()=>requestAnimationFrame(r)));window.print();});
  window.addEventListener("afterprint",()=>{document.body.classList.remove("v19125-druckt","v19125-print-quer","v19126-kosten-druckt");ausrichtungsStil.textContent="";});
  vorschau();
})();
